...
wnd_t wp_pick_window(int* keys, int* cancel_keys, int poll_ms)
{
    ...
    if (!GetCursorPos(&pos))
    {
        wp_err("GetCursorPos failed, GLE=%08X");
        return 0;
    }
...
int update_window_styles(wnd_t wnd, long _and, long ex_and, long _or, long ex_or)
{
    unsigned gle;
...
errcheck:
    ...
    if ((!style || !exstyle) && gle) {
        wp_err("GetWindowLongA failed, GLE=%08X");
        return 1;
    }
...
int wp_add(wnd_t wnd)
{
    ...
    *wndclass = 0;
    ...
    if (!SetParent(wnd, wallpaper)) {
        wp_err("SetParent failed, GLE=%08X");
        return 1;
    }
...
int wp_del(wnd_t wnd)
{
    ...
    _or = WS_OVERLAPPEDWINDOW;
    ex_or = WS_EX_APPWINDOW;

    if (update_window_styles(wnd, -1, -1, _or, ex_or)) {
        return 1;
    }
...
int wp_kill(wnd_t wnd)
{
    ...
    if (!process) {
        wp_err("OpenProcess failed, GLE=%08X");
        return 1;
    }

    if (!TerminateProcess(process, 0)) {
        wp_err("TerminateProcess failed, GLE=%08X");
        return 1;
    }
...
int wp_get_rect(wnd_t wnd, rect_t* rect)
{
    if (!GetWindowRect(wnd, (RECT*)rect)) {
        wp_err("GetWindowRect failed, GLE=%08X");
        return 1;
    }
    return 0;
}
...
int wp_move(wnd_t wnd, long left, long top, long right, long bottom)
{
    ...
    if (!succ) {
        wp_err("SetWindowPos failed, GLE=%08X");
        return 1;
    }
    return 0;
}
...
int wp_fullscreen(wnd_t wnd)
{
    ...
    if (!mon) {
        wp_err("MonitorFromWindow failed, GLE=%08X");
        return 1;
    }
    ...
    if (!GetMonitorInfoA(mon, &mi)) {
        wp_err("GetMonitorInfoA failed, GLE=%08X");
        return 1;
    }
...
int wp_exec(char const* file, char const* params)
{
    if (!ShellExecuteA(0, "open", file, params, 0, 0)) {
        wp_err("ShellExecuteA filed, GLE=%08X");
        return 1;
    }
...
handle_t wp_mutex(char const* name)
{
    ...
    if (!res) {
        wp_err("CreateMutexA failed, GLE=%08X");
    }
